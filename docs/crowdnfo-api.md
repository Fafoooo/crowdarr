...
crowdarr deliberately keeps every CrowdNFO route and auth header in
...
| --- | --- | --- |
| Test profile-key route | `GET /api/releases/__crowdarr_connection_test__/files/best` | A 2xx verifies access; 404 is reachable but authentication-indeterminate; 401/403 is invalid |
...
as `X-Api-Key`. GET requests are limited to two concurrent calls, lightly paced,
and retried with exponential backoff for 429/5xx and network failures; uploads are
not retried automatically because their idempotency cannot be assumed.

## Byte preservation

The best-file call returns a `fileId`; crowdarr then downloads from the file
...
2. **Swagger auth mismatch:** the generated Swagger security description and
   ...
   `X-Api-Key` profile-key header. In particular, `/api/user/me` rejects profile
   keys, while best-file/download routes accept them. crowdarr therefore never
   reports a 404 probe as proof that a key is valid.
3. **Release creation is not contribution:** `POST /api/releases` is restricted
   ...
   `filelists` routes listed above; crowdarr does not attempt release creation.
...
5. **One best NFO response:** the current lookup has no relative-path selector.
   When a torrent reports several missing NFO entries, crowdarr places the one
